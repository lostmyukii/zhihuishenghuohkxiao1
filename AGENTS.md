...
| `设计方案.md` | 规则审计与作品定位。 |
| `assets/wiring/n16r8-primary-wiring.png` | 旧版 image 模型连线示意图，可能仍含 8键/RGB/风扇/舵机或 GPIO48 继电器，精确端口以本文档为准。 |
...
如果图片局部小字和 `开发文档.md` 表格不一致，以 `开发文档.md` 和实板丝印复核为准。当前硬件没有 8键AD、RGB 灯带、风扇和舵机，不再把这些模块作为验收项。用户实板 `LY-AS-ESP32-S3 v1.2` 未找到可用 `GPIO48` 白色接口，学习灯/继电器已改接 `D12 / GPIO12`。
