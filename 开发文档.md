...
红色告警卡片只能来自 `telemetry.alerts`，不能直接由原始传感器值在网页上猜出来。当前核心告警建议：
...
告警文案必须保留未知码，例如 `设备上报异常：<code>`，不能静默丢弃。没有新鲜 `telemetry` 时显示“等待实时数据”或“离线/重连中”，不得显示假正常。
...
下一检查点：验证 `D5 / GPIO5` PIR 节能联动。在 `energy` 节能模式和较暗环境下，确认“有人时允许开灯、无人时自动关灯”，并核对 Dashboard 的人体状态、学习灯状态、节能分和节能原因。
