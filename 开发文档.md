...
当前公网入口为 `https://hongkongxiao.ilelezhan.cn/`。小学服务使用独立目录、独立域名和隔离端口，不影响服务器上已有系统：

| 端口/路径 | 角色 |
| --- | --- |
| `127.0.0.1:19267` | Dashboard 静态服务 |
| `127.0.0.1:19266` / `/smartlife-primary-ws` | WSS Cloud Relay |
| `127.0.0.1:19283` | 独立 MQTT Broker |

三个后端端口不直接暴露公网，外部统一通过 Nginx `443` 的 HTTPS/WSS 访问。初中系统的 `19166/19167/19168/19183` 不修改、不重启。
...
### 2026-07-12：公网 WSS/MQTT 与 macOS 实板远端同步

公网地址：`https://hongkongxiao.ilelezhan.cn/`。小学服务部署在 `/home/ubuntu/smartlife-primary`，使用 `19266` Relay、`19267` Dashboard 和 `19283` MQTT，均只监听 `127.0.0.1`。

| 验收项 | 结果 | 现场证据 |
| --- | --- | --- |
| DNS 与 HTTPS | 通过 | 域名解析到部署服务器；Let's Encrypt 独立证书签发成功并启用自动续期。 |
| 静态资源一致性 | 通过 | 公网 `index.html`、JS 和 CSS 共 7 个文件逐一与本地 SHA-256 一致。 |
| WSS 与 MQTT | 通过 | 公网 WSS 返回 `101 Switching Protocols`；测试 telemetry 经 WSS 写入 MQTT retained 并回到 Dashboard。 |
| 离线真实性 | 通过 | 停止测试帧后约 `3.5s`，页面恢复“开发板离线”“等待实时数据”，不保留假在线。 |
| macOS 公网 USB | 通过 | Chrome 公网页面连接真实 `/dev/cu.usbserial-120`，第二个无 USB 浏览器读到固件 `0.1.2` 和连续实板温湿度、光照、噪声、PIR 数据。 |
| Windows 浏览器兼容 | 通过 | Windows 11 Chrome 149 用户代理下，HTTPS 安全上下文、`navigator.serial`、WSS、真实远端 `0.1.2` 数据和 `1366px` 布局均正常；此项不替代真实 `COMx` 授权。 |
| 远端命令回写 | 通过 | 无 USB 浏览器下发休息模式，实板返回 `ack mode=rest`；学习灯关闭、蜂鸣器静音。随后远端恢复学习模式并收到 `ack mode=study`。 |
| 手机布局 | 通过 | `390px` 宽度无横向溢出，云端状态可读，浏览器控制台零错误。 |
| 服务隔离 | 通过 | 三个 `smartlife-primary-*` 服务 active；四个 `smartlife-junior-*` 服务保持 active，Nginx 检查通过。 |

本检查点证明公网双向链路已经打通：

```text
macOS Chrome + N16R8 USB -> WSS -> MQTT -> 无 USB 浏览器实时显示
无 USB 浏览器命令 -> WSS/MQTT -> USB Chrome -> N16R8 -> ack/telemetry -> 两端同步
```

下一检查点：在真实 Windows 10/11 Chrome 或 Edge 上连接 `USB-SERIAL CH340 (COMx)`，验证公网授权、实时 `hello/telemetry`、模式命令和 `ack`。页面兼容与云端链路已通过，Windows 实板仍需现场机器完成最后确认。
